...
void saveToFile(Node* head, int size, const string& filename, bool isBinary) {
    // Открываем файл либо в бинарном, либо в текстовом режиме
    ...
    if (isBinary) {
        ...
        while (curr != nullptr) {
            ...
            // Остальные данные пишем напрямую из памяти
...
void loadFromFile(Node*& head, Node*& tail, int& size, const string& filename, bool isBinary) {
    ...
    clearList(head, tail, size); // Очищаем старый список
...
void createFile(bool isBinaryMode) {
    ...
    // Открываем файл для записи (это автоматически создаст его)
    ...
    // Записываем 0, чтобы файл считался корректной пустой базой данных
    int initialSize = 0;
    if (isBinaryMode) {
